    ...
    def __getitem__(self, index):
        ...
        if self.opt.c2f or self.opt.c2f_sem or self.opt.c2f_sem_rec:
            ...
            if self.opt.aspect_ratio == 2.0 and self.opt.load_size == 512:
                smap35 = torch.squeeze(torch.nn.functional.interpolate(coarse_fine, size=(35, 67), mode='nearest'),
                                       dim=0)
                smap19 = torch.squeeze(torch.nn.functional.interpolate(coarse_fine, size=(19, 35), mode='nearest'),
                                       dim=0)
            elif self.opt.aspect_ratio == 1.0:  # the conditions should be completed
                smap35 = torch.squeeze(torch.nn.functional.interpolate(coarse_fine, size=(35, 35), mode='nearest'),
                                       dim=0)
                smap19 = torch.squeeze(torch.nn.functional.interpolate(coarse_fine, size=(19, 19), mode='nearest'),
                                       dim=0)
            else:
        ...
        tmp = ((label_map_segmentation == 0) + (label_map_segmentation == 1) + (label_map_segmentation == 2) + (
                label_map_segmentation == 3) + (label_map_segmentation == 4) + (label_map_segmentation == 5) + (
                       label_map_segmentation == 6) + (label_map_segmentation == 9) + (label_map_segmentation == 10) \
               + (label_map_segmentation == 14) + (label_map_segmentation == 15) + (label_map_segmentation == 16) + (
                       label_map_segmentation == 18) + (label_map_segmentation == 29) + (
                       label_map_segmentation == 30) + (label_map_segmentation == -1)).type(torch.bool)
