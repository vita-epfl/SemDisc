...
class GANLoss(nn.Module):
    ...
    def __call__(self, input, target_is_real, weight=None, for_discriminator=True):
        ...
        if self.wmse:
            ...
            if isinstance(input, list):
                ...
                for pred_i in input:
                    ...
                    loss_tensor = self.loss(pred_i, target_is_real, weight, for_discriminator)
                    bs = 1 if len(loss_tensor.size()) == 0 else loss_tensor.size(0)
                    new_loss = torch.mean(loss_tensor.view(bs, -1), dim=1)
                    loss += new_loss
                return loss / len(input)
