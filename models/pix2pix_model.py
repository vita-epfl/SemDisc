...
class Pix2PixModel(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        if opt.isTrain:
            self.criterionGAN = networks.GANLoss(
                ...
                wmse=(self.opt.c2f_sem_rec or self.opt.c2f or self.opt.c2f_sem))
            self.criterionFeat = torch.nn.L1Loss()
            self.criterionSeg = torch.nn.CrossEntropyLoss(ignore_index=255)
            self.upsample = torch.nn.Upsample(
                size=(int(self.opt.crop_size // self.opt.aspect_ratio), self.opt.crop_size), mode='bilinear')
    ...
    def compute_generator_loss(self, input_semantics, real_image, smap35=None, smap19=None, fine_tune=False,
                               label_mapS=None):
        ...
        if self.opt.c2f_sem:
            if type(pred_fakeS) == list:
                ...
                for seg in pred_fakeS:  # different discriminators for different scales
                    ...
                    G_losses['Seg'] = G_losses['Seg'] + self.criterionSeg(seg, label_mapS) * self.opt.lambda_seg * (
                            0.0 + self.opt.active_GSeg)
            else:
                pred_fakeS = self.upsample(pred_fakeS)
                G_losses['Seg'] = self.criterionSeg(pred_fakeS, label_mapS) * self.opt.lambda_seg * (
                    self.opt.active_GSeg)
        elif self.opt.c2f_sem_rec:
            if type(pred_fakeS) == list:
                G_losses['Seg'] = 0.0
                for seg in pred_fakeSelf:  # different discriminators for different scales
                    seg = self.upsample(seg)
                    G_losses['Seg'] = G_losses['Seg'] + self.criterionFeat(seg, real_image) * self.opt.lambda_rec * (
                            0.0 + self.opt.active_GSeg)
                for seg in pred_fakeS:  # different discriminators for different scales
                    seg = self.upsample(seg)
                    G_losses['Seg'] = G_losses['Seg'] + self.criterionSeg(seg, label_mapS) * self.opt.lambda_seg * (
    ...
    def generate_fake(self, input_semantics, real_image, compute_kld_loss=False):
        ...
        if self.opt.netG == 'asapnets':
            ...
            if self.opt.D_output:
                pred_fake, pred_real, pred_fakeS, pred_realS = self.discriminate(input_semantics, fake_image.detach(),
                                                                                 real_image)
    ...
    def discriminate(self, input_semantics, fake_image, real_image):
        ...
        if self.opt.c2f_sem:
            ...
            pred_fake, pred_real, pred_fakeS, pred_realS, pred_fakeSelf, pred_realSelf = self.divide_pred_seg_rec(
                discriminator_out)
